...
guide_text_main = Text(
    ...
    Pre('Нажимаете на кнопку только тогда, когда идёте срать. Всем  чат (группы, если таковая имеется) отправляется уведомление, что вы идёте срать. Под уведомлением появляется кнопка "отменить автозавершение сранья". При её нажатии автоматическое завершение акта дефекации через 10 минут будет отключено.', language='Я иду срать'), '\n\n',
